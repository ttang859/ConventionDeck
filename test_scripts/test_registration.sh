...
UID=$(jq -r '.id' ./test_scripts/script_output.json)

echo "Convention setup"
jq --arg key "host_id" --arg value "$UID" '. += {($key): $value}' ./test_data/create_convention.json > ./test_data/temp.json && mv ./test_data/temp.json ./test_data/create_convention.json
...
curl -X POST http://localhost:8080/registration/register_user -H "Content-Type: application/json" -d '{"conv_id":"'"$CONV_ID"'", "attendee_id":"'"$UID"'"}'
echo -e "\n"
...
echo "Testing get attendees by conv_id"
curl -X POST http://localhost:8080/registration/get_attendees/$UID
echo -e "\n"

echo "Testing unregister attendee"
curl -X DELETE http://localhost:8080/registration/unregister_user -H "Content-Type: application/json" -d '{"conv_id":"'"$CONV_ID"'", "attendee_id":"'"$UID"'"}'
echo -e "\n"
