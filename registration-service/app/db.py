...
def unregister_user(conv_id, attendee_id):
    with Session(engine) as session, session.begin():
        statement = select(Registration).where(and_(Registration.conv_id == conv_id, Registration.attendee_id == attendee_id))
...
def get_convention_attendees(conv_id):
    with Session(engine) as session:
        ...
        return list(map((lambda a: RegisteredPair(conv_id=a.conv_id, attendee_id=a.attendee_id)), attendees))

def get_attending_conventions(attendee_id):
    with Session(engine) as session:
        statement = select(Registration).where(Registration.attendee_id == attendee_id)
        conventions = session.exec(statement)
        return list(map((lambda a: RegisteredPair(conv_id=a.conv_id, attendee_id=a.attendee_id)), conventions))
